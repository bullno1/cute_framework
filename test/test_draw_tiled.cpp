...
// -------------------------------------------------------------------------------------------------
// Read-only lookup of a layer for the tests below: NULL when the layer does not exist.
...
{
	for (int i = 0; i < s_draw->sorted_layers.count(); ++i) {
		if (s_draw->sorted_layers[i]->layer == layer) return s_draw->sorted_layers[i];
	}
	return NULL;
}
...
// -------------------------------------------------------------------------------------------------
// Replaying a draw list routes its commands to the layers they were recorded on (a command
// opened right after cf_draw_push_layer belongs to the pushed layer), creating layers as
...
static bool s_layered_list_replayed_ok;
...
static void s_scene_list_replay_layers()
{
	...
	s_layered_list_replayed_ok = lower && lower->cmds.count() && lower->cmds[0].geoms_ref
...
TEST_CASE(test_draw_list_replay_layers)
{
	...
	cf_draw_pop_layer();
	cf_draw_list_end();
	// Recording left the live layers alone: the cursor is back on the default layer.
	...
	for (int mode = 0; mode <= 1; ++mode) {
		...
		REQUIRE(s_layered_list_replayed_ok);
...
// -------------------------------------------------------------------------------------------------
// Layers persist empty across frames while in use and are pruned once a whole frame passes
// without a command on them, so one-layer-per-object schemes don't accumulate layers forever.
