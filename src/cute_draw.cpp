...
{
	int lo = 0;
	int hi = sorted_layers.count();
	while (lo < hi) {
		int mid = (lo + hi) >> 1;
		if (sorted_layers[mid]->layer < layer) lo = mid + 1;
		else hi = mid;
	}
	if (lo < sorted_layers.count() && sorted_layers[lo]->layer == layer) return sorted_layers[lo];
	// Sorted insert of a fresh heap layer: pointers already handed out (notably `current_layer`) stay
	// valid, only this array of pointers shifts.
	CF_DrawLayer* dl = CF_NEW(CF_DrawLayer);
	dl->layer = layer;
	sorted_layers.add(NULL);
	for (int i = sorted_layers.count() - 1; i > lo; --i) {
		sorted_layers[i] = sorted_layers[i - 1];
	}
	sorted_layers[lo] = dl;
	return dl;
}
...
{
	if (current_layer) {
		// Detach the non-drawing tail of the layer being left. Empty commands
		// are simply dropped.
	...
	for (int i = 0; i < carry_scratch.count(); ++i) {
		...
		moved.layer = layer; // It belongs to this layer now (draw list replay routes by this field).
...
void CF_Draw::clear_layers()
{
	for (int q = 0; q < sorted_layers.count(); ++q) {
		Cute::Array<CF_Command>& cmds = sorted_layers[q]->cmds;
...
void CF_Draw::prune_idle_layers()
{
	int w = 0;
	for (int q = 0; q < sorted_layers.count(); ++q) {
		CF_DrawLayer* dl = sorted_layers[q];
		if (!dl->touched) {
			CF_ASSERT(dl->cmds.count() == 0);
			dl->~CF_DrawLayer();
			CF_FREE(dl);
			continue;
		}
	...
	sorted_layers.set_count(w);
	current_layer = NULL;
}

void CF_Draw::free_layers(Cute::Array<CF_DrawLayer*>& draw_layers)
{
	for (int i = 0; i < draw_layers.count(); ++i) {
		draw_layers[i]->~CF_DrawLayer();
		CF_FREE(draw_layers[i]);
	}
	draw_layers.clear();
}
...
void cf_destroy_draw()
{
	...
	CF_Draw::free_layers(s_draw->sorted_layers);
...
CF_CustomShape cf_make_custom_shape(const char* sdf_src)
{
	...
	for (int q = 0; q < s_draw->sorted_layers.count(); ++q) {
		Cute::Array<CF_Command>& cmds = s_draw->sorted_layers[q]->cmds;
		for (int i = 0; i < cmds.count(); ++i) {
...
void cf_draw_list_begin(CF_DrawList list)
{
	...
	CF_ASSERT(s_draw->recording_saved_layers.count() == 0);
	swap(s_draw->sorted_layers, s_draw->recording_saved_layers);
...
void cf_draw_list_end()
{
	...
	cf_draw3d_free_list_cmds(data);
	data->cmds.clear();
	// Walk the recording's layers in layer order: the list keeps its commands grouped by
	// layer, in record order within each, which is exactly how cf_draw_list routes them.
	for (int q = 0; q < s_draw->sorted_layers.count(); ++q) {
		Cute::Array<CF_Command>& recorded = s_draw->sorted_layers[q]->cmds;
	...
	// Drop the recording's layers and restore the live scene's.
	CF_Draw::free_layers(s_draw->sorted_layers);
	swap(s_draw->sorted_layers, s_draw->recording_saved_layers);
...
void cf_draw_list(CF_DrawList list)
{
	...
	for (int i = 0; i < data->cmds.count(); ++i) {
		...
		if (src.mesh3d) {
			...
			bool adjacent = prev_mesh_index >= 0 && prev_mesh_index == dl->cmds.count() - 2;
			CF_Command* prev = adjacent ? &dl->cmds[prev_mesh_index] : NULL;
			if (cf_draw3d_replay_cmd(&c, prev, &src)) {
				// Fused into the previous replayed command: discard the one just added.
				dl->cmds.pop();
			} else {
				prev_mesh_index = dl->cmds.count() - 1;
			}
		}
	}
...
// First command of the next non-empty layer after index `q` whose layer id is still within
...
{
	for (int i = q + 1; i < s_draw->sorted_layers.count(); ++i) {
		CF_DrawLayer* dl = s_draw->sorted_layers[i];
		if (dl->layer > layer_hi) return NULL;
		if (dl->cmds.count()) return &dl->cmds[0];
	}
	return NULL;
}
...
void cf_render_layers_to(CF_Canvas canvas, int layer_lo, int layer_hi, bool clear)
{
	...
	// order, so there is no need to sort: the flush walks the in-range layers low to high.
	// State change commands are moved to the head of the target layer's queue on layer
	// switch.
	int first = 0;
	while (first < s_draw->sorted_layers.count() && s_draw->sorted_layers[first]->layer < layer_lo) ++first;
	int last = first;
	while (last < s_draw->sorted_layers.count() && s_draw->sorted_layers[last]->layer <= layer_hi) ++last;
	...
	// Process each rendering command, layer by layer.
	for (int q = first; q < last; ++q) {
		Cute::Array<CF_Command>& cmds = s_draw->sorted_layers[q]->cmds;
	...
	// Release the processed commands. Out-of-range layers keep theirs for a later flush.
	for (int q = first; q < last; ++q) {
		Cute::Array<CF_Command>& cmds = s_draw->sorted_layers[q]->cmds;
		for (int i = 0; i < cmds.count(); ++i) {
			cf_draw3d_free_cmd(&cmds[i]);
